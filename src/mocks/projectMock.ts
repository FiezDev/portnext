...
export const SideProjects: SideProjectObj[] = [
  ...
  },
  {
    ...
    projectDesc: [
      ...
      'Fully on the edge now: two Cloudflare Workers — an approval gate (LINE push, signed webhook, monthly push caps) and the bot itself (D1 conversations, Vectorize + FTS5 hybrid retrieval fused with RRF and rerank, generation driven by a cron convergence loop behind a lease). The site proxies same-origin so the bot URL and secret never reach the browser; requests are HMAC-signed, replay-nonce-guarded, rate-limited, and carry a correlation id through every hop.',
