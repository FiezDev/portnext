...
export function useHeroGame(): HeroGame {
  ...
  const start = useCallback(() => {
    ...
    const words = pickGameWords(RELATED_WORDS, GAME_WORD_COUNT, mulberry32(newSeed));
    const targetRng = mulberry32((newSeed ^ 0x9e3779b9) >>> 0);
    const first = pickNextTarget(words, null, targetRng);
  ...
  const registerHit = useCallback((word: string) => {
    ...
    if (word === s.target) {
      ...
      let remaining = s.words.filter((w) => !s.hit.has(w));
      if (remaining.length === 0) {
        // Cleared the board — refill so play can continue until the timer ends.
        s.hit.clear();
        remaining = [...s.words];
      }
      s.target = pickNextTarget(remaining, word, s.targetRng);
